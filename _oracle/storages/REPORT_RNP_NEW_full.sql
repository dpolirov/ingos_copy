CREATE OR REPLACE PACKAGE "STORAGES"."REPORT_RNP_NEW" IS
  ...
  PROCEDURE rpt_rnp_all(pTaskIsn in number); -- интерфейс с утилитой автомат. обновления отчетов
  ...
  PROCEDURE make_rnp_re_rsbu_6654(pLoadIsn NUMBER, pDateRep DATE);
  PROCEDURE make_rnp_re_rsbu_6693(pLoadIsn NUMBER, pDateRep DATE);
...
CREATE OR REPLACE PACKAGE BODY "STORAGES"."REPORT_RNP_NEW" IS
  ...
  PROCEDURE rpt_rnp_all(pTaskIsn in number)
  IS
  BEGIN
    make_rnp_all;
  END;
  ...
  PROCEDURE make_rnp_re_rsbu_6693(ploadisn IN NUMBER, pDateRep IN DATE)
  IS
    --vReISN CONSTANT NUMBER := 35283655303;
    /*TYPE RnpReRSBURecType IS RECORD(
      reid VARCHAR2(255), --2
      redatebase VARCHAR2(1), --3
      reclassisn NUMBER, -- 4
      sectfullname VARCHAR2(255), -- 5
      secttype NUMBER, -- 6
      sectdatebeg DATE, -- 7
      sectdateeend DATE, -- 8
      sectcurrisn NUMBER, -- 9
      objisn NUMBER, -- 10
      riskisn NUMBER, -- 11
      agrid VARCHAR2(255), -- 12
      rernp NUMBER, -- 16
      agrisn NUMBER, -- 25
      reisn NUMBER -- 26
    );*/
    --RnpReRSBURec RnpReRSBURecType;

    --TYPE RnpReRsbuTabType IS TABLE OF RnpReRSBURecType;
    --RnpReRsbuTab RnpReRsbuTabType;
  BEGIN
    NULL;
  END;

  PROCEDURE make_rnp_re_rsbu_6654(ploadisn IN NUMBER, pDateRep IN DATE)
  IS

    --vReISN CONSTANT NUMBER := 33607555003;
    /*TYPE RnpReRSBURecType IS RECORD(
      reid VARCHAR2(255), --2
      redatebase VARCHAR2(1), --3
      reclassisn NUMBER, -- 4
  ...
  --код работал до 27.01.2012. новая реализация см. выше процедуру make_rnp_re_msfo_virtualcond
  /*PROCEDURE make_rnp_re_msfo_virtualcond(pLoadisn IN NUMBER)
  IS
    vMinIsn     NUMBER :=-1;
    vMaxIsn     NUMBER;
    ...
    TYPE TAgrTab IS TABLE OF NUMBER INDEX BY BINARY_INTEGER;
    AgrTab TAgrTab;

  BEGIN
    SesId:=Parallel_Tasks.createnewsession;

    --{EGAO 14.12.2011 до этого не было партиций у таблицы rnp_re_msfo_virtualcond_buffer
    \*EXECUTE IMMEDIATE 'truncate table storages.rnp_re_msfo_virtualcond_buffer';
    store_and_drop_table_index('storages.rnp_re_msfo_virtualcond_buffer', 1);
    *\
    vPart:=init_partition_by_key(pTableName => 'storages.rnp_re_msfo_virtualcond_buffer',pKey => pLoadIsn,pCompress => 1);
    --}
    replog_i (pLoadIsn, 'LoadRNPNEW', 'rnp_re_msfo_virtualcond_buffer',pAction=>'Begin');
    LOOP
      vMaxIsn:=Cut_Table('storages.rep_agrre','agrisn',vMinIsn,pRowCount=>vLoadObjCnt);
      ...
      vSql:= 'DECLARE
                ...
                vCnt    number :='||vCnt||';
              BEGIN
                DBMS_APPLICATION_INFO.SET_MODULE(''rnp_re_msfo'',''VirtualCondBuffer. Precess#''||vCNT);
                pparam.clear;
                pparam.SetParamN(''MinAgrIsn'',vMinIsn);
                pparam.SetParamN(''MaxAgrIsn'',vMaxIsn);

                INSERT into storages.rnp_re_msfo_virtualcond_buffer (
                  loadisn,
                  condisn,
                  agrisn,
                  datebeg,
                ...
                SELECT vLoadIsn, condisn, agrisn, datebeg,dateend
                FROM storages.v_rnp_re_msfo_virtualcond_buf;
    ...
    --restore_table_index('storages.rnp_re_msfo_virtualcond_buffer',0);
    replog_i (pLoadIsn, 'LoadRNPNEW', 'rnp_re_msfo_virtualcond_buffer',pAction=>'End');


    replog_i (pLoadIsn, 'LoadRNPNEW', 'make_rnp_re_msfo_vcond_by_agr',pAction=>'Begin');
    vPart:=init_partition_by_key(pTableName => 'storages.rnp_re_msfo_virtualcond',pKey => pLoadIsn,pCompress => 1);
    SELECT a.agrisn
    BULK COLLECT INTO AgrTab
    FROM (
          SELECT \*+ FULL ( a ) *\ agrisn, COUNT(1) AS cnt
          FROM rnp_re_msfo_virtualcond_buffer a
    ...
    IF AgrTab.count<>0 THEN
      SesId:=Parallel_Tasks.createnewsession;
      FOR i IN AgrTab.FIRST .. AgrTab.LAST LOOP
        vSql:= 'DECLARE
                    vAgrIsn number :='||AgrTab(i)||';
                    vCnt    number :='||i||';
                    vLoadIsn number :='||pLoadisn||';
                  BEGIN
                    DBMS_APPLICATION_INFO.SET_MODULE(''rnp_re_msfo'',''VirtualCondMain. Process:''||vCNT);
                    storages.report_rnp_new.make_rnp_re_msfo_vcond_by_agr(vLoadisn, vAgrIsn);
                  END;';
        System.Parallel_Tasks.processtask(sesid,vsql);
        DBMS_APPLICATION_INFO.set_module('rnp_re_msfo','VirtualCondMain. Applied: '||i);
      END LOOP;
      -- ждем, пока завершатся все джобы
      Parallel_Tasks.endsession(sesid);
    END IF;
    replog_i (pLoadIsn, 'LoadRNPNEW', 'make_rnp_re_msfo_vcond_by_agr',pAction=>'End');

  END;*/

  --код работал до 27.01.2012. новая реализация см. выше процедуру make_rnp_re_msfo_virtualcond
  /*PROCEDURE make_rnp_re_msfo_vcond_by_agr(pLoadisn IN NUMBER, pAgrIsn IN NUMBER)
  IS
    vDateRep DATE := GetDateRep(pLoadIsn);
  BEGIN
    IF vDateRep IS NULL THEN
    ...
    pparam.Clear;
    pparam.SetParamN('AgrIsn',pagrisn);
    pparam.SetParamN('LoadIsn',pLoadisn);
    INSERT INTO rnp_re_msfo_virtualcond_old(isn,
                                        condisn,
                                        agrisn,
                                        datebeg,
                                        dateend,
                                        ...
                                        virtualdateend, loadisn)
    SELECT seq_recond_msfo.nextval, a.condisn, a.agrisn, a.datebeg, a.dateend, a.sharepc,
           vDateRep, a.virtualdatebeg, a.virtualdateend, a.loadisn
    FROM v_rnp_re_msfo_virtualcond_old a;
    COMMIT;
  END;*/
