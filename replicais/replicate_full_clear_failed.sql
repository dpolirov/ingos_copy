create or replace function replicais.replicate_full_clear_failed() returns void as $$
declare
...
begin
    select array_agg(r.replication_task_isn), array_agg(r.replication_table)
        ...
        from (select replication_task_isn, replication_table, replication_job_isn
                from replicais.replication_tasks_full
                where replication_job_isn is not null and replication_status = 'RUN') as r 
                    left join 
                        (select job_isn
                            from dbms_jobs.job_queue
                            where job_status = 'FLD') as j
