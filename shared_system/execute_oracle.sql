 ...
 */
create or replace function shared_system.execute_oracle(requisites varchar, query varchar) returns varchar as $BODY$
try:
    import cx_Oracle
 ...
 *      query       - query that will be executed on remote DBMS
 */
create or replace function shared_system.execute_oracle(os_conn_id int, query varchar) returns varchar as $BODY$
declare
    p_username varchar;
    p_password varchar;
...
begin
    ...
    if p_username is null then
        ...
        select shared_system.execute_oracle(p_username || '/' || p_password || '@' || p_server_name || '/' || p_database_name, query)
            into res;
    end if;    
    return res;
end;
