...
alias pga="psql -c 'select * from pg_stat_activity;'"
alias pgg="date; psql -c 'select * from pg_stat_activity;' | grep -v '<IDLE>'"
alias pg="psql -c 'select * from pg_stat_activity;' | grep -v '<IDLE>'"
...
alias gjobs="psql -c 'select q.job_isn, g.group_name, g.priority, q.job_submit_dttm, q.job_session_start_dttm, q.job_status from dbms_jobs.job_queue as q inner join dbms_jobs.job_group as g on q.job_group=g.group_isn order by q.job_submit_dttm limit 20;'"
