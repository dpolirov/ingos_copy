create or replace view storage_adm.v_repagr (
   agrisn,
   id,
   datebeg,
   dateend,
...
(select --+ ordered use_hash(sd  fil bfil) use_nl(ar)
      ...
      from ( select --+ ordered use_nl(r a ar dr drp ad sd sdf sdf1 arc clnt) use_hash(agnt gm)
                    ...
                    decode(a.currisn, 53, a.premiumsum, shared_system.gcc2(a.premiumsum, a.currisn, 53::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone)) premusd,
                    decode(a.currisn, 35, a.premiumsum, shared_system.gcc2(a.premiumsum, a.currisn, 35::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone)) premrub,
                    decode(a.currisn, 29448516, a.premiumsum, shared_system.gcc2(a.premiumsum, a.currisn, 29448516::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone)) premeur,
                    ...
                    null buhdate,
                    a.limitsum,
                    decode(a.currisn, 53, a.limitsum, shared_system.gcc2(a.limitsum, a.currisn, 53::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone)) limitsumusd,
                    decode(a.currisn, 35, a.limitsum, shared_system.gcc2(a.limitsum, a.currisn, 35::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone)) limitsumrub,
                    a.insuredsum,
                    decode(a.currisn, 53, a.insuredsum, shared_system.gcc2(a.insuredsum, a.currisn, 53::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone)) insuredsumusd,
                    decode(a.currisn, 35, a.insuredsum, shared_system.gcc2(a.insuredsum, a.currisn, 35::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone)) insuredsumrub,
                    ...
                    -- sts 19.10.2012 - task(38397275003)
                    a.incomesum,
                    shared_system.gcc2(a.incomesum, a.currisn, 53::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone) as incomesumusd,
                    shared_system.gcc2(a.incomesum, a.currisn, 35::numeric, least(a.datesign, oracompat.trunc(current_timestamp), a.datebeg)::timestamp without time zone) as incomesumrub,
                    -- sts 06.11.2012 - скидка для туристов
                    a.discount,
               ...
               from storage_adm.tt_rowid r
                        ...
                        on a.ruleisn = dr.isn
                        left join storage_source.agr_detail_agrhash ad
                        on a.isn = ad.agrisn
                        left join ( select --+ ordered use_nl(x) index(x x_agrext_agr) use_hash(d)
                                        ...
                                        from storage_adm.tt_rowid r,
                                             ...
                                             ( select isn
                                                 from ais.dicti_nh z
                                                 where shared_system.is_subtree(__hier, 2255842303) -- продукты gm
              ...
              where a.discr in ('Д', 'Г')
                and a.classisn in ( select isn
                                      from ais.dicti_nh
                                     where shared_system.is_subtree(__hier, 34711216) -- тип договора страхования
    ...
    /* kgs  24.10.11 нафиг не нужен тут этот гроупбай
              group by a.isn, a.id, a.datebeg, a.dateend, a.datesign, a.classisn, a.ruleisn,
                       ...
                       a.datecalc */) s
                        left join storage_source.repagrroleagr ar
                        on s.agrisn = ar.agrisn
                        left join storage_source.rep_dept sd
                        on s.deptisn = sd.deptisn
                        left join storage_source.rep_dept fil
                        on ar.emitisn = fil.deptisn
                        left join storage_source.rep_dept bfil
