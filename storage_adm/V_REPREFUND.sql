...
(/* kgs 10.01.2013    repcond нельзя использовать - в нем теперь нет медиц кондов с 0 плановой премией*/
    ...
    --!!       oracompat.nvl(rc.parentobjisn,rc.objisn) parentobjisn,
           ...
           ac.franchcurrisn,
           oracompat.nvl(decode(oracompat.nvl(ac.franchtype, 'Б'), 'Б', decode (ac.franchtariff, null, shared_system.gcc2(ac.franchsum, ac.franchcurrisn, s.currisn, oracompat.nvl(oracompat.nvl(dateloss, dateevent), datereg)))), 0::numeric) +
           oracompat.nvl(s.claimsum * decode(oracompat.nvl(ac.franchtype, 'Б'), 'Б', ac.franchtariff), 0::numeric) / 100 franchdeducted,
           s.classisn,
           s.refundsum,
           s.refundsumusd,
           s.claimsumusd,
                         ...
                         from (
                               select --+ ordered use_nl ( t cl r ext cr )
                                   ...
                                   where r.emplisn not in ( select --+ index (sb x_subject_class)
                                        ...
                                        and oracompat.nvl(cl.classisn, 0::numeric) <> 2835056703 -- акция "помощь друга" od 27.11.2009 12475086503
                              ) s 
