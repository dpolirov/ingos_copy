...
begin
    insert into storage_adm.replog(loadisn, module, operation, action, objcount, sqltext) values
    (pLoadIsn, pModule, pOperation, pAction, pObjCount, pSqlText);
end;
$BODY$
language plpgsql;
...
begin
    insert into storage_adm.replog(loadisn, module, operation, action) values
    (pLoadIsn, pModule, pOperation, pAction);
end;
$BODY$
language plpgsql;
...
-- 
--------------------------------------------------------------------------------
create or replace function STORAGE_ADM.LOADSTORAGE(pProcIsn int, pFull smallint) returns void as $BODY$
declare
    vLoadIsn        numeric;
...
begin
    ...
    for v_dest_tbl In (Select * from ss_proces_dest_tables Where Procisn = pProcIsn order by PRIORITY Asc nulls first) loop
        if pFull = 0 then
            --incremental load
            v_step = Cur.TABLE_NAME||' Insert tt';
            RepLog_i (pLoadIsn, 'Load '||pProcName||' By_tt_RowId', v_step,  'Begin');
            ...
            RepLog_i (pLoadIsn, 'Load '||pProcName||' By_tt_RowId', v_step,  'End');
            ...
            v_step = Cur.TABLE_NAME||' Delete deleted';
            RepLog_i (pLoadIsn, 'Load '||pProcName||' By_tt_RowId', v_step,  'Begin');
            vSql =
            ...
            execute vSql;
            GET DIAGNOSTICS vRc = ROW_COUNT;
            RepLog_i (pLoadIsn, 'Load '||pProcName||' By_tt_RowId', v_step,  'End', vRc, null);
            ...
            --here was inserting by blocks in loop  /second part of load_tt_by_rowid procedure
            execute 'truncate table tt_rowid_r';
            execute 'Insert into tt_rowid_r(RId)
                    ...
                    'Delete 
                        ...
                        Where ('||v_dest_tbl.KeyField||')  In
                            ('||Get_Minus_slq(  v_dest_tbl.TABLE_NAME,
                                                ...
                                                'Where   ('||v_dest_tbl.KeyField||') in (
                                                    Select '||v_dest_tbl.KeyField||' 
                                                        from tt_rowid_r tt, '||v_dest_tbl.TT_TABLE_NAME||' t 
                                                        Where t.rowid =tt.rId )',
                                                'Where   rowid in (select rId from tt_rowid_r)'
                                              )||
                           ');';
            ...
            vsql:='
                ...
                Select nextval(''storage_adm.ss_seq''), Isn, '||pLoadIsn||',S.*
                    FROM (
                        ...
                        (
                            SELECT *
                                FROM (' ||
                                    Get_Minus_slq(v_dest_tbl.TT_TABLE_NAME,
                                                  v_dest_tbl.TABLE_NAME,
                                                  v_dest_tbl.KeyField,
                                                  'Where rowid  in (select rId from tt_rowid_r r ) ',
                                                  'Where   ('||v_dest_tbl.KeyField||') in (
                ...
                RepLog_i ('||pLoadIsn||', ''Load '||pProcName||' By_tt_RowId'', '''||v_dest_tbl.TABLE_NAME||' DeleteInsert'',  pAction => ''End'');
            '; 
            ...
            vSql:='
                RepLog_i ('||pLoadIsn||', ''Load Full'||pProcName||' By_tt_RowId'', '''||v_dest_tbl.TABLE_NAME||'Insert'',  pAction => ''Begin'');

                Insert Into '||v_dest_tbl.TABLE_NAME||'
                Select nextval(''storage_adm.ss_seq''), '||pLoadIsn||', S.*
                    From (select * from '||v_dest_tbl.VIEW_NAME||') s;

                RepLog_i ('||pLoadIsn||', ''Load Full'||pProcName||' By_tt_RowId'', '''||v_dest_tbl.TABLE_NAME||'Insert'',  pAction => ''End'');';

            execute vSql;
        end if;
        
        RepLog_i (pLoadIsn, 'Load '||pProcName||' By_tt_RowId', v_dest_tbl.TABLE_NAME||' Insert tt',  pAction => 'End', pBlockIsn=>vBlockIsn);
    end loop;  --by dest tables

    
    If (pFull=0) then
        -- clear log
        RepLog_i (pLoadIsn, 'Load '||pProcName||' By_tt_RowId', ' Clear Log',  pAction => 'Begin');
        Update SS_BUF_LOG H
        ...
        RepLog_i (pLoadIsn, 'Load '||pProcName||' By_tt_RowId', ' Clear Log',  pAction => 'End');
    end if;
